...
# ---------- 1. Load data ----------
df = pd.read_csv("CS13Dataset.csv")
...
plt.title("Decision Tree - Heart Disease Risk Prediction (full detail)", fontsize=18)
plt.tight_layout()
plt.savefig("CS13_decision_tree.png", dpi=300, bbox_inches="tight")
plt.show()
...
print(tree_text)
with open("CS13_decision_tree_rules.txt", "w") as f:
    f.write(tree_text)
...
try:
    ...
    graph.render("CS13_decision_tree_graphviz", format="png", cleanup=True)
    print("\nHigh-res Graphviz tree saved as CS13_decision_tree_graphviz.png")
except ImportError:
